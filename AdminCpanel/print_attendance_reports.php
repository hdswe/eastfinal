...
<table class="table table-hidaction table-striped mb30">
		...
		<th>الحي</th>
				...
				if($count_course_days>0){
					$rows_day=$pdo->pdoGetAll($sql_course_days);
            ...
            <tbody>
                ...
                $sql = "SELECT 
                        ...
                        FROM reservation, users,neighborhood,center
				...
				$sql_attend="SELECT * FROM `registr_attend` WHERE day_course_id=".$result_day['id']." and user_id=".$result['userid']." and course_id=".$id." ";
				$execute_sql_attend = $pdo->pdoExecute($sql_attend);
				$user_attend = $pdo->pdoRowCount($execute_sql_attend);
				if($user_attend==1){
					?>
					<td><? echo '<span class="glyphicon glyphicon-ok"></span>'; ?></td>
				<?
				}else{
					?>
					<td><? echo '<span class="glyphicon glyphicon-remove"></span>'; ?></td>
					<?
				}}}
				?>
