...
#############################################################
    ...
    if(isset($_POST['btnadd'])) {
        if($chkToken->Check()) { 
                ...
                $insert = $pdo->pdoInsUpd('mosque', $data);
...
#############################################################
    if($_GET['do'] == 'edit' and isset($_GET['id'])){
        $sql = "SELECT * FROM `mosque` where id=:id";
        $data[id] = $_GET['id'];
    ...
    if(isset($_POST['btnedit'])) {
        if($chkToken->Check()) { 
                $data['title']		          = trim($_POST['title']);
                $where[id] = $_GET['id'];
                $update = $pdo->pdoInsUpd('mosque', $data, 'update', $where);
                header('Location: ?do=show&process=successfully');
            } else { 
               die(header("Location: login.php")); 
        }  
...
<section>
  ...
  <div class="mainpanel">
    ...
    <?
        ...
        {
        case "show":
        include("template/mosque/show.php");
        break;
        case "add":
        include("template/mosque/add.php");
        break;
        case "edit":
        include("template/mosque/edit.php");
        break;
        }
