...
#############################################################
    if($_FILES['fileimage']['name'] != ""){
		...
		if(empty($error))
		{
			...
			$destFile_thumbs = "../data/images/" . $uploadedfileimage;
			$destFile_orignal = "../data/images/" . $uploadedfileimage;

    
            $resizeObja = new resize($srcFile);
            $resizeObja -> resizeImage(450, 'landscape');		
            $resizeObja -> saveImage($destFile_thumbs, 100);

            $resizeObja = new resize($destFile_thumbs);
            $resizeObja -> resizeImage(450, 160 , 'crop');		
...
#############################################################
    ...
    if(isset($_POST['btnadd'])) {
        if($chkToken->Check()) { 
                ...
                $insert = $pdo->pdoInsUpd('slide', $data);
...
#############################################################
    ...
    if(isset($_POST['btnedit'])) {
        if($chkToken->Check()) { 
                ...
                $data['image']          = $uploadedfileimage;
                $where[id] = $_GET['id'];
                $update = $pdo->pdoInsUpd('slide', $data, 'update', $where);
...
<section>
  ...
  <div class="mainpanel">
    ...
    <?
        ...
        {
        case "show":
        include("template/slide/show.php");
        break;
        case "add":
        include("template/slide/add.php");
        break;
        case "edit":
        include("template/slide/edit.php");
        break;
        }
